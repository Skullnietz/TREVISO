...
<div class="space-y-4">
  <h1 class="text-xl font-bold text-lt-text dark:text-dk-text flex items-center gap-2">
    <span class="material-symbols-outlined text-primary">currency_exchange</span> Reembolsos
  </h1>
  ...
  <div class="bg-lt-surface dark:bg-dk-surface rounded-xl border border-lt-border dark:border-dk-border shadow-sm overflow-hidden">
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-lt-border dark:divide-dk-border">
          ...
          <tr class="hover:bg-primary/10 transition-colors">
            <td class="px-4 py-3 text-xs whitespace-nowrap">{{$r->FechaReembolso}}</td>
            <td class="px-4 py-3 text-xs">{{$r->cliente->NombreCliente??'-'}}</td>
            <td class="px-4 py-3 text-xs">{{$r->NombreBancoReembolso}}</td>
            <td class="px-4 py-3 text-xs">{{$r->TerminacionBancoReembolso}}</td>
            <td class="px-4 py-3 text-xs">{{$r->ReferenciaBancoReembolso}}</td>
            <td class="px-4 py-3 text-right font-medium">${{ number_format($r->TotalReembolso,2) }}</td>
            <td class="px-4 py-3 text-center">
              @if($r->Contabilizado==='S')<span class="material-symbols-outlined text-sm text-green-600">check_circle</span>
    ...
    <div class="px-4 py-3 border-t border-lt-border dark:border-dk-border">{{ $reembolsos->links() }}</div>
  </div>
</div>
@endsection